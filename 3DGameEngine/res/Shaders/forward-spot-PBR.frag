...
void main(){
    ...
    vec3 lightDir = spotLight.pointLight.position - worldPos0;
    float distanceToLight = length(lightDir);

    
    lightDir = normalize(lightDir);
    float cutoffAngle = dot(lightDir, normalize(spotLight.direction));
    ...
    vec3 outRadiance = (diffuse * albedo * color / PI + specular) * spotLight.pointLight.base.color * angle;  

    float attenuation = 1 / (spotLight.pointLight.attenuation.exponent * distanceToLight * distanceToLight +
                        spotLight.pointLight.attenuation.linear * distanceToLight +
                        spotLight.pointLight.attenuation.constant + 0.00001); 
    ...
    colorOut = colorOut * (1.0 -(1.0 - angle) / (1.0 - spotLight.cutoff));
    if(distanceToLight > spotLight.pointLight.range || cutoffAngle < spotLight.cutoff)
