...
IndexedModel OBJModel::ToIndexedModel()
{
	...
	for (unsigned int i = 0; i < numIndices; i++)
	{
		...
		math::Vector3 currentPosition = vertices[currentIndex->vertexIndex];
		math::Vector2 currentTexCoord;
		math::Vector3 currentNormal;

		if (hasUVs)
			currentTexCoord = uvs[currentIndex->uvIndex];
		else
			currentTexCoord = math::Vector2(0, 0);

		if (hasNormals)
			currentNormal = normals[currentIndex->normalIndex];
		else
			currentNormal = math::Vector3(0, 0, 0);
...
math::Vector3 OBJModel::ParseOBJVec3(const std::string& line)
{
	...
	//math::Vector3(atof(tokens[1].c_str()), atof(tokens[2].c_str()), atof(tokens[3].c_str()))
}

math::Vector2 OBJModel::ParseOBJVec2(const std::string& line)
{
	unsigned int tokenLength = line.length();
	const char* tokenString = line.c_str();
