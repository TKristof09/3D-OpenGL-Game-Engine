...
namespace quickhull {
	...
	template<typename T>
	class ConvexHull {
		std::unique_ptr<std::vector<math::Vector3>> m_optimizedVertexBuffer;
	...
	public:
		...
		ConvexHull(const ConvexHull& o) {
			...
			if (o.m_optimizedVertexBuffer) {
				m_optimizedVertexBuffer.reset(new std::vector<math::Vector3>(*o.m_optimizedVertexBuffer));
		...
		ConvexHull(const MeshBuilder<T>& mesh, const VertexDataSource<T>& pointCloud, bool CCW, bool useOriginalIndices) {
			if (!useOriginalIndices) {
				m_optimizedVertexBuffer.reset(new std::vector<math::Vector3>());
			}
			
			std::vector<bool> faceProcessed(mesh.m_faces.size(),false);
			std::vector<std::size_t> faceStack;
			std::unordered_map<std::size_t,std::size_t> vertexIndexMapping; // Map vertex indices from original point cloud to the new mesh vertex indices
			for (std::size_t i = 0;i<mesh.m_faces.size();i++) {
			...
			const std::size_t finalMeshFaceCount = mesh.m_faces.size() - mesh.m_disabledFaces.size();
			...
			while (faceStack.size()) {
				...
				if (faceProcessed[top]) {
					...
					std::size_t adjacent[] = {mesh.m_halfEdges[mesh.m_halfEdges[halfEdges[0]].m_opp].m_face,mesh.m_halfEdges[mesh.m_halfEdges[halfEdges[1]].m_opp].m_face,mesh.m_halfEdges[mesh.m_halfEdges[halfEdges[2]].m_opp].m_face};
					for (auto a : adjacent) {
		...
		std::vector<std::size_t>& getIndexBuffer() {
			return m_indices;
		}
