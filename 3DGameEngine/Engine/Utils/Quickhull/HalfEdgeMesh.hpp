...
namespace quickhull {
	...
	struct Vertex {
		...
		IndexType m_halfEdge;
	};
	//TODO remake the structs to contain pointers instead of indexes
	...
	public:
				...
				if (!face.isDisabled()) {
					...
					for (const auto heIndex : heIndices) {
						...
						if (vertexMapping.count(vertexIndex)==0) {
							Vertex v = { vertexData[vertexIndex], heIndex };
		...
		//TODO optimize
		Vertex FindVertexByPosition(const math::Vector3& position) const{
			for (Vertex vertex : m_vertices)
			{
				if(vertex.m_position == position)
					return vertex;
			}
		}
