...
namespace quickhull {
	...
	struct DiagnosticsData {
		std::size_t m_failedHorizonEdges; // How many times QuickHull failed to solve the horizon edge. Failures lead to degenerated convex hulls.
	...
	class QuickHull {
		...
		bool m_planar;
		std::vector<math::Vector3> m_planarPointCloudTemp;
	...
	public:
		...
		ConvexHull<FloatType> getConvexHull(const std::vector<math::Vector3>& pointCloud, bool CCW, bool useOriginalIndices, FloatType eps = Epsilon);
		...
		//      then we generate a new vertex buffer which contains only the vertices that are part of the convex hull.
		//   eps: minimum distance to a plane to consider a point being on positive of it (for a point cloud with scale 1)
		ConvexHull<FloatType> getConvexHull(const math::Vector3* vertexData, std::size_t vertexCount, bool CCW, bool useOriginalIndices, FloatType eps = Epsilon);
		...
		//      then we generate a new vertex buffer which contains only the vertices that are part of the convex hull.
		//   eps: minimum distance to a plane to consider a point being on positive of it (for a point cloud with scale 1)
		ConvexHull<FloatType> getConvexHull(const FloatType* vertexData, std::size_t vertexCount, bool CCW, bool useOriginalIndices, FloatType eps = Epsilon);
		...
		HalfEdgeMesh<FloatType, IndexType> getConvexHullAsMesh(const FloatType* vertexData, std::size_t vertexCount, bool CCW, FloatType eps = Epsilon);
	...
	void QuickHull<T>::reclaimToIndexVectorPool(std::unique_ptr<std::vector<IndexType>>& ptr) {
		const std::size_t oldSize = ptr->size();
