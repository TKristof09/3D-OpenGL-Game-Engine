...
namespace quickhull {
	...
	template<typename T>
	ConvexHull<T> QuickHull<T>::getConvexHull(const std::vector<math::Vector3>& pointCloud, bool CCW, bool useOriginalIndices, T epsilon) {
		VertexDataSource<T> vertexDataSource(pointCloud);
		return getConvexHull(vertexDataSource,CCW,useOriginalIndices,epsilon);
	}
	
	template<typename T>
	ConvexHull<T> QuickHull<T>::getConvexHull(const math::Vector3* vertexData, std::size_t vertexCount, bool CCW, bool useOriginalIndices, T epsilon) {
		VertexDataSource<T> vertexDataSource(vertexData,vertexCount);
		return getConvexHull(vertexDataSource,CCW,useOriginalIndices,epsilon);
	}
	
	template<typename T>
	ConvexHull<T> QuickHull<T>::getConvexHull(const T* vertexData, std::size_t vertexCount, bool CCW, bool useOriginalIndices, T epsilon) {
		VertexDataSource<T> vertexDataSource((const math::Vector3*)vertexData,vertexCount);
		return getConvexHull(vertexDataSource,CCW,useOriginalIndices,epsilon);
	}
	
	template<typename FloatType>
	HalfEdgeMesh<FloatType, IndexType> QuickHull<FloatType>::getConvexHullAsMesh(const FloatType* vertexData, std::size_t vertexCount, bool CCW, FloatType epsilon) {
		VertexDataSource<FloatType> vertexDataSource((const math::Vector3*)vertexData,vertexCount);
	...
	void QuickHull<T>::buildMesh(const VertexDataSource<T>& pointCloud, bool CCW, bool useOriginalIndices, T epsilon) {
		...
		if (m_planar) {
			const std::size_t extraPointIndex = m_planarPointCloudTemp.size()-1;
	...
	void QuickHull<T>::createConvexHalfEdgeMesh() {
		...
		while (!faceList.empty()) {
			iter++;
			if (iter == std::numeric_limits<std::size_t>::max()) {
			...
			const math::Vector3& activePoint = m_vertexData[tf.m_mostDistantPoint];
			const std::size_t activePointIndex = tf.m_mostDistantPoint;
			...
			visibleFaces.clear();
			possiblyVisibleFaces.emplace_back(topFaceIndex,std::numeric_limits<std::size_t>::max());
			while (possiblyVisibleFaces.size()) {
				...
				if (pvf.m_visibilityCheckedOnIteration == iter) {
					...
					const T d = math::dot(P.m_N,activePoint)+P.m_D;
					if (d>0) {
			...
			const std::size_t horizonEdgeCount = horizonEdges.size();
			...
			if (disableCounter < horizonEdgeCount*2) {
				const std::size_t newHalfEdgesNeeded = horizonEdgeCount*2-disableCounter;
				for (std::size_t i=0;i<newHalfEdgesNeeded;i++) {
					m_newHalfEdgeIndices.push_back(m_mesh.addHalfEdge());
				}
			}
			
			// Create new faces using the edgeloop
			for (std::size_t i = 0; i < horizonEdgeCount; i++) {
				const IndexType AB = horizonEdges[i];

				auto horizonEdgeVertexIndices = m_mesh.getVertexIndicesOfHalfEdge(m_mesh.m_halfEdges[AB]);
			
				IndexType A = horizonEdgeVertexIndices[0];
				IndexType B = horizonEdgeVertexIndices[1];
				IndexType C = activePointIndex;
				...
				const math::Vector3 planeNormal = mathutils::getTriangleNormal(m_vertexData[A],m_vertexData[B],activePoint);
			...
			for (auto& disabledPoints : m_disabledFacePointVectors) {
				...
				for (const auto& point : *(disabledPoints)) {
					...
					for (std::size_t j=0;j<horizonEdgeCount;j++) {
	...
	std::array<IndexType,6> QuickHull<T>::getExtremeValues() {
		...
		for (std::size_t i=1;i<vCount;i++) {
			const math::Vector3& pos = m_vertexData[i];
	...
	bool QuickHull<T>::reorderHorizonEdges(std::vector<IndexType>& horizonEdges) {
		const std::size_t horizonEdgeCount = horizonEdges.size();
		for (std::size_t i=0;i<horizonEdgeCount-1;i++) {
			const IndexType endVertex = m_mesh.m_halfEdges[ horizonEdges[i] ].m_endVertex;
			bool foundNext = false;
			for (std::size_t j=i+1;j<horizonEdgeCount;j++) {
	...
	MeshBuilder<T> QuickHull<T>::getInitialTetrahedron() {
		...
		if (vertexCount <= 4) {
			IndexType v[4] = {0,std::min((std::size_t)1,vertexCount-1),std::min((std::size_t)2,vertexCount-1),std::min((std::size_t)3,vertexCount-1)};
			const math::Vector3 N = mathutils::getTriangleNormal(m_vertexData[v[0]],m_vertexData[v[1]],m_vertexData[v[2]]);
		...
		for (std::size_t i=0;i<6;i++) {
			for (std::size_t j=i+1;j<6;j++) {
				const T d = math::distanceSq(m_vertexData[m_extremeValues[i]], m_vertexData[m_extremeValues[j]]);
				if (d > maxD) {
					maxD=d;
		...
		if (maxD == m_epsilonSquared) {
			...
			return MeshBuilder<T>(0,std::min((std::size_t)1,vertexCount-1),std::min((std::size_t)2,vertexCount-1),std::min((std::size_t)3,vertexCount-1));
		}
		...
		if (maxD == m_epsilonSquared) {
			...
			auto it = std::find_if(m_vertexData.begin(),m_vertexData.end(),[&](const math::Vector3& ve) {
			...
			it = std::find_if(m_vertexData.begin(),m_vertexData.end(),[&](const math::Vector3& ve) {
		...
		std::array<std::size_t,3> baseTriangle{selectedPoints.first, selectedPoints.second, maxI};
		const math::Vector3 baseTriangleVertices[]={ m_vertexData[baseTriangle[0]], m_vertexData[baseTriangle[1]],  m_vertexData[baseTriangle[2]] };
		...
		maxD=m_epsilon;
		maxI=0;
		const math::Vector3 N = mathutils::getTriangleNormal(baseTriangleVertices[0],baseTriangleVertices[1],baseTriangleVertices[2]);
		...
		if (maxD == m_epsilon) {
			...
			m_planar = true;
			const math::Vector3 N = mathutils::getTriangleNormal(baseTriangleVertices[1],baseTriangleVertices[2],baseTriangleVertices[0]);
			m_planarPointCloudTemp.clear();
			m_planarPointCloudTemp.insert(m_planarPointCloudTemp.begin(),m_vertexData.begin(),m_vertexData.end());
			const math::Vector3 extraPoint = N + m_vertexData[0];
		...
		for (auto& f : mesh.m_faces) {
			...
			const math::Vector3& va = m_vertexData[v[0]];
			const math::Vector3& vb = m_vertexData[v[1]];
			const math::Vector3& vc = m_vertexData[v[2]];
			const math::Vector3 N = mathutils::getTriangleNormal(va, vb, vc);
